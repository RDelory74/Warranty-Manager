  ...
  try {
    ...
    <template>
      <UContainer class="py-10">
        <UCard>
          <template #header>
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">Nouvelle demande de SAV</h1>
          </template>
          ...
          <form @submit.prevent="submitTicket" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="flex flex-col gap-1.5">
                ...
                <UInput v-model="form.customer_name" required placeholder="Jean Dupont" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="text-sm font-medium text-gray-700 dark:text-gray-200">Téléphone</label>
                <UInput v-model="form.customer_phone" placeholder="06..." />
              </div>
            </div>
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <UTextarea v-model="form.product_problem" required placeholder="Décrivez le défaut constaté..." />
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <USelect v-model="form.requested_solution" :options="solutionOptions" />
              </div>
            </div>
            ...
            <div class="flex flex-col gap-1.5 border-t border-gray-100 dark:border-gray-800 pt-4">
              ...
              <input type="file" multiple accept="image/*" @change="handleFileChange" class="text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 cursor-pointer" />
              
              <div v-if="previews.length" class="flex gap-4 mt-2">
                <div v-for="(src, index) in previews" :key="index" class="relative">
                  <img :src="src" class="w-20 h-20 object-cover rounded-lg border-2 border-primary-500" />
                </div>
              </div>
            </div>
    
            <div class="flex justify-end gap-3 pt-4">
              <UButton color="info" variant="soft" @click="router.back()">Annuler</UButton>
              <UButton type="submit" :loading="loading" color="primary" icon="i-heroicons-paper-airplane">Envoyer le dossier</UButton>
            </div>
          </form>
        </UCard>
      </UContainer>
